...
class Settings(BaseSettings):
    ...
    # CPU PaddleOCR can take 25-30 seconds on first/warm complex screenshots.
    timeout_seconds: float = Field(default=90.0, gt=0, le=600)
    min_confidence: float = Field(default=0.50, ge=0, le=1)
    # Paddle's CPU inference is resource-heavy; serial requests avoid timeout
    # cascades where a timed-out worker still occupies its inference slot.
